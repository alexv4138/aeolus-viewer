...
function formatDate(value: string) { return new Date(`${value.slice(0, 10)}T12:00:00`).toLocaleDateString('ro-RO', { day: '2-digit', month: 'short', year: 'numeric' }); }
...
function MiniBars({ points, field, color = '#18201e', large = false }: { points: Point[]; field: keyof Point; color?: string; large?: boolean }) {
  ...
  return <div className={`mini-bars ${large ? 'large' : ''}`} aria-hidden="true">{values.map((value, index) => <span key={`${index}-${value}`} title={`${formatDateTime(points[index].DataOra)}: ${format(value)}`} style={{ height: `${Math.max(8, value / max * 100)}%`, backgroundColor: color }} />)}</div>;
}
...
function ChartPanel({ label, sublabel, points, field, color, wide, onOpen }: { label: string; sublabel: string; points: Point[]; field: keyof Point; color?: string; wide?: boolean; onOpen?: () => void }) {
  if (!points.length) return <section className={`chart-panel chart-empty ${wide ? 'wide' : ''}`}><div className="chart-title"><strong>{label}</strong><span>0 citiri</span></div><p>Nu există citiri pentru intervalul ales.</p></section>;
  const values = points.map((point) => Number(point[field])); const min = Math.min(...values); const max = Math.max(...values);
  return <button type="button" className={`chart-panel ${wide ? 'wide' : ''}`} onClick={onOpen}><div className="chart-title"><strong>{label}</strong><span>{sublabel}</span></div><MiniBars points={points} field={field} color={color} /><div className="chart-axis"><span>{formatDateTime(points[0].DataOra)}</span><span>{points.length} citiri · min {format(min)} · max {format(max)}</span><span>{formatDateTime(points.at(-1)!.DataOra)}</span></div></button>;
}
...
export default function Home() {
  ...
  const allPoints = [...(records[selectedTurbine?.locationId] ?? referenceHistory)].sort((a, b) => new Date(a.DataOra).getTime() - new Date(b.DataOra).getTime());
  ...
  const points = validRange ? allPoints.filter((point) => (!fromDate || point.DataOra.slice(0,10) >= fromDate) && (!toDate || point.DataOra.slice(0,10) <= toDate)) : [];
  const latest = (allPoints.at(-1) ?? referenceHistory[0])!;
  const popupValues = popup ? points.map((point) => Number(point[popup.field])) : [];
  const popupMin = popupValues.length ? Math.min(...popupValues) : 0;
  const popupMax = popupValues.length ? Math.max(...popupValues) : 0;
  function exportData() { if (!points.length) return; const rows = [['Turbină','Locație','ID locație','Data/Ora','Temperatură aer','Presiune atmosferică','Umiditate','Viteză vânt','Direcție vânt','Radiație solară','Turație','Voltaj','Amperaj','Putere','Energie','Vibrații','Cuplu mecanic','Temperatura vântului','Alarmă'], ...points.map(p=>[selectedTurbine.id,selectedTurbine.location,p.IDLocatie,p.DataOra,p.TempC,p.PresAtm,p.Umiditate,p.VitVant,p.DirectieVant,p.RadSolara,p.Turatie,p.Voltaj,p.Amperaj,p.Putere,p.Energie,p.Vibratii,p.CupluMec,p.TempInfas,p.Alarma ? 'Da' : 'Nu'])]; const csv='\uFEFF'+rows.map(r=>r.map(v=>`"${String(v).replaceAll('"','""')}"`).join(';')).join('\r\n'); const url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'})); const a=document.createElement('a'); a.href=url; a.download=`telemetrie-${selectedTurbine.id.toLowerCase().replaceAll(' ','-')}-${fromDate || availableFrom}-${toDate || availableTo}.csv`; a.click(); URL.revokeObjectURL(url); }
