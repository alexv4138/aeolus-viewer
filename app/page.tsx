...
function MiniBars({ points, field, color = '#18201e' }: { points: Point[]; field: keyof Point; color?: string }) {
  const values = points.map((point) => Number(point[field])); const max = Math.max(...values, 1);
  return <div className="mini-bars" aria-hidden="true">{values.map((value, index) => <span key={`${index}-${value}`} style={{ height: `${Math.max(8, value / max * 100)}%`, backgroundColor: color }} />)}</div>;
}
...
export default function Home() {
  ...
  const points = records[selectedTurbine?.locationId] ?? referenceHistory; const latest = points.at(-1)!;
